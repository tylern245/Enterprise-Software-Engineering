...
<?php
    ...
    if (isset($_POST['upload-file']))
    {
        ...
        $sql = "INSERT INTO `documentsCRON` (`loanNum`, `docType`, `fileName`, `uploadBy`, `uploadDate`, `status`, `fileType`, `content`, `fileSize`) VALUES ('', '', '$fileName','$uploadBy','$uploadDate','active','$docType','$contentsClean','$fileSize')";
            ...
            <div class="panel-body">
                ...
                <form method="post" action="">
                    ...
                    <select name="searchType">
                        ...
                        <option value="loanNum">Loan Number</option>
                ...
                if (isset($_POST['search-file'])) {            
                    ...
                    switch($searchType) {
                        case "name":
                            $sql="SELECT `auto_id`, `loanNum`, `docType`, `fileName`, `uploadDate`, `uploadBy`, `fileSize` FROM `documentsCRON` WHERE `fileName` LIKE '%$searchString%'";
                            break;
                        case "loanNum":
                            $sql="SELECT `auto_id`, `loanNum`, `docType`, `fileName`, `uploadDate`, `uploadBy`, `fileSize` FROM `documentsCRON` WHERE `loanNum` LIKE '%$searchString%'";
                            break;
                        case "docType":
                            $sql="SELECT `auto_id`, `loanNum`, `docType`, `fileName`, `uploadDate`, `uploadBy`, `fileSize` FROM `documentsCRON` WHERE `docType` LIKE '%$searchString%'";
                            break;                                        
                        case "uploadBy":
                            $sql="SELECT `auto_id`, `loanNum`, `docType`, `fileName`, `uploadDate`, `uploadBy`, `fileSize` FROM `documentsCRON` WHERE `uploadBy` LIKE '%$searchString%'";
                            break;
                        case "uploadDate":
                            $sql="SELECT `auto_id`, `loanNum`, `docType`, `fileName`, `uploadDate`, `uploadBy`, `fileSize` FROM `documentsCRON` WHERE `uploadDate` LIKE '%$searchString%'";
                            break;
                        case "all":
                            $sql="SELECT `auto_id`, `loanNum`, `docType`, `fileName`, `uploadDate`, `uploadBy`, `fileSize` FROM `documentsCRON`";
                            break;
                        default:
                ...
                ?>
                        <tr>
                            ...
                            <td><?php echo $data['loanNum'] ?></td>
                            <td><?php echo $data['docType'] ?></td>
                            <td><?php echo $data['fileName'] ?></td>
                            <td><?php echo $data['uploadDate'] ?></td>
                            <td><?php echo $data['uploadBy'] ?></td>
